...
Write-Log "Start " "D:\PowerShell\Immagini\log.lista.media.mov.TLG.txt" 
...
for ($row = 2; $row -le 1001; $row++) {
    ...
    if (($valore5 -eq 'Y') -and $valore2 -and $valore3) {
        $identificatori = "${valore2}_${valore3}_*.mov"
        Write-Log "Added $valore2 $valore3" "D:\PowerShell\Immagini\log.lista.media.mov.TLG.txt" 
    
            # Crea la sottocartella se non esiste
        if (-not (Test-Path -Path $destinationPath)) {
            New-Item -ItemType Directory -Path $destinationPath | Out-Null
            Write-Log "Created Folder " "D:\PowerShell\Immagini\log.lista.media.mov.TLG.txt" 
...
            Write-Host "Nessun file trovato con il pattern specificato."
            Write-Log "Nessun file trovato con il pattern specificato." "D:\PowerShell\Immagini\log.lista.media.mov.TLG.txt" 
        } else {
            # Stampa i file trovati
            Write-Host "File trovati:"
            Write-Log "File trovati:" "D:\PowerShell\Immagini\log.lista.media.mov.TLG.txt" 
...
            Copy-Item -Path $file.FullName -Destination $destinationPath -Force
            ...
            Write-Log "Copiato :  $fileTemp " "D:\PowerShell\Immagini\log.lista.media.mov.TLG.txt" 
...
Write-Log "Finish " "D:\PowerShell\Immagini\log.lista.media.mov.TLG.txt" 
